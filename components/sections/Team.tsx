  ...
  return (
    <section id="team" className="py-20 md:py-32 bg-kinertic-black">
      <div className="max-w-7xl mx-auto px-6">
        <SectionHeader
          title="Meet Our Team"
          subtitle="Not freelancers. A conversion-focused pod for strategy, creative, and growth."
          centered
        />
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-10">
          {['200+ Projects Delivered', '98% Client Retention', 'Weekly Performance Reporting'].map((item) => (
            <div
              key={item}
              className="px-4 py-2 rounded-full border border-kinertic-gold/30 bg-kinertic-gold/10 text-sm text-gray-200"
            >
              {item}
            </div>
          ))}
          ...
          {team.map((member, index) => (
            <ScrollReveal key={index} variant="fadeUp" delay={index * 0.1}>
              <Card hover className="text-center h-full">
                ...
                <p className="text-gray-200 text-sm leading-relaxed font-medium mb-4">
                  {member.impact}
                </p>
                <span className="inline-flex items-center rounded-full bg-kinertic-gold/10 border border-kinertic-gold/30 px-2.5 py-1 text-xs uppercase tracking-[0.12em] text-kinertic-gold mb-6">
                  {member.market}
                </span>
